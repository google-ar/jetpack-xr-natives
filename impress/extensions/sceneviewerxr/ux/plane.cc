...
#include "extensions/sceneviewerxr/assets/plane_assets.h"
...
constexpr float kSnapHeightThreshold = 0.05f;
...
constexpr auto kGlowDuration = absl::Milliseconds(200);
...
Future<OwnedTexturePtr> LoadImageTexture(imp::BaseView& view) {
  return view.GetAssetManager()
      .LoadImage(imp::kDotPattern64Png)
      .Then([&view](AssetPtr<ImageAsset> image) {
        return view.GetTextureFactory().CreateTexture(image);
      });
}
...
bool Plane::ShouldBeActive(imp::float3 target_position_world) const {
  ...
  if (fabs(target_position_local.y) < kSnapHeightThreshold) {
    return false;
  }
...
void Plane::Activate(imp::float3 target_position_world) {
  ...
  machine_.UpdateWithAlternatives(
      ...
      [this](plane_interaction_states::Active& state)
          -> InteractionMachine::OptionalState {
        if (state.glow.GetTarget() != 1) {
          state.glow.SetTarget(1, kGlowDuration);
        }
...
void Plane::CalculateSnappability(imp::float3 target_position_world) {
  auto target_position_local =
      GetNode()->LocalFromWorldPoint(target_position_world);
  is_snappable_ = fabs(target_position_local.y) < kSnapHeightThreshold;
}
