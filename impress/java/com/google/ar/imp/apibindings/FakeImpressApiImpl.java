...
public class FakeImpressApiImpl implements ImpressApi {
  ...
  // warning, however, we get a build error - (broken link).
  ...
  /** Test data about Impress nodes. */
  public static class ImpressNodeData {
    int mEntityId;
    ...
    // This is slightly different from how the real API behaves, because not all Impress nodes
    // host GLTFs (for example SubspaceNodes and SurfaceEntities)
  ...
  private final Map<ImpressNodeData, ImpressNodeData> mImpressNodes = new HashMap<>();
  ...
  public ListenableFuture<Long> loadImageBasedLightingAsset(@NonNull String path) {
    ...
    // TODO: Enforce minSDK API strategy - (broken link)
  ...
  public int instanceGltfModel(long gltfToken, boolean enableCollider) {
    ...
    ImpressNodeData impressNodeData = new ImpressNodeData();
    impressNodeData.setEntityId(entityId);
    mImpressNodes.put(impressNodeData, null);
    return entityId;
  }
  ...
  public ListenableFuture<Void> animateGltfModel(
    ...
    if (getImpressNodeData(impressNode) == null) {
  ...
  public void destroyImpressNode(int impressNode) {
    ImpressNodeData impressNodeData = getImpressNodeData(impressNode);
    if (impressNodeData == null) {
      throw new IllegalArgumentException("Impress node not found");
    }
    ...
    for (Map.Entry<ImpressNodeData, ImpressNodeData> pair : mImpressNodes.entrySet()) {
      if (pair.getValue() != null && pair.getValue().equals(impressNodeData)) {
        pair.setValue(null);
      }
    }
    mImpressNodes.remove(impressNodeData);
  ...
  public void setImpressNodeParent(int impressNodeChild, int impressNodeParent) {
    ImpressNodeData childImpressNodeData = getImpressNodeData(impressNodeChild);
    ImpressNodeData parentImpressNodeData = getImpressNodeData(impressNodeParent);
    if (childImpressNodeData == null || parentImpressNodeData == null) {
      throw new IllegalArgumentException("Impress node(s) not found");
    }
    mImpressNodes.put(childImpressNodeData, parentImpressNodeData);
  }
  ...
  public boolean impressNodeHasParent(int impressNode) {
    ...
    return mImpressNodes.get(impressNodeData) != null;
  }

  /** Returns the parent impress node for the given impress node. */
  public int getImpressNodeParent(int impressNode) {
    ImpressNodeData impressNodeData = getImpressNodeData(impressNode);
    ImpressNodeData parentImpressNodeData = mImpressNodes.get(impressNodeData);
    if (impressNodeData == null || parentImpressNodeData == null) {
      return -1;
    }
    return parentImpressNodeData.mEntityId;
  }
  ...
  // TODO - (broken link): Set the content security level properly.
  @Override
  ...
  public ListenableFuture<WaterMaterial> createWaterMaterial(boolean isAlphaMapVersion) {
    ...
    WaterMaterial material =
        new WaterMaterial.Builder().setImpressApi(this).setNativeMaterial(materialToken).build();
  ...
  public ListenableFuture<KhronosPbrMaterial> createKhronosPbrMaterial(
    ...
    KhronosPbrMaterial material =
        new KhronosPbrMaterial.Builder()
            ...
            .build();
    mMaterials.put(materialToken, new MaterialData(MaterialData.Type.KHRONOS_PBR, materialToken));
  ...
  public void setMaterialOverride(int impressNode, long nativeMaterial, @NonNull String meshName) {
    ...
    impressNodeData.setMaterialOverride(mMaterials.get(nativeMaterial));
  }
  ...
  @NonNull
  public Map<ImpressNodeData, ImpressNodeData> getImpressNodes() {
  ...
  @Nullable
  private ImpressNodeData getImpressNodeData(int impressNode) {
    for (Map.Entry<ImpressNodeData, ImpressNodeData> pair : mImpressNodes.entrySet()) {
