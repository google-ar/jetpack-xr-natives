...
namespace imp {
WasmPlatformCanvasSource::WasmPlatformCanvasSource(BaseView& view)
    : wrapped_(absl::WrapUnique(new WasmAsyncCanvasSource(view))) {}
...
std::unique_ptr<ScopedCanvas> WasmPlatformCanvasSource::StartDrawing(
  ...
  return wrapped_->StartDrawing(pixel_size, draw_mode);
}
...
std::unique_ptr<ScopedCanvas> WasmPlatformCanvasSource::StartDrawing(
  ...
  return wrapped_->StartDrawing(pixel_size, std::move(on_texture_changed_fn),
                                draw_mode, loc);
}
