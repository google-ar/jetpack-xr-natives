...
#ifndef THIRD_PARTY_IMPRESS_CORE_VIEW_FRAMEWORK_COLLISION_CAPSULE_COLLIDER_H_
#define THIRD_PARTY_IMPRESS_CORE_VIEW_FRAMEWORK_COLLISION_CAPSULE_COLLIDER_H_
...
class CapsuleCollider : public Component,
                        public ColliderMaskHelpers<CapsuleCollider> {
 public:
  void Setup();
  void Setup(const Capsule& capsule);
  void SetupWithState();

  void Cleanup();

  void SetCapsule(const Capsule& capsule);
  void SetCapsule(float3 center, float height, float radius);
  ...
  // ComponentSystem for registering the CapsuleCollider to the relevant
  // CollisionSystem
  class System : public ComponentSystem<CapsuleCollider> {
   public:
    explicit System(BaseView* view) : ComponentSystem<CapsuleCollider>(view) {}

    void BeforeFirstComponentAdded() override;
  };

 private:
  friend class ColliderMaskHelpers<CapsuleCollider>;
  CapsuleColliderState state_;
  Flags<CollisionMask> collision_flags_{CollisionMask::kDefault};
  NodeHandle hit_node_;

 public:
  using IsfInfo = IsfInfo<&CapsuleCollider::state_>;
  static constexpr bool kRunInEditMode = true;
};
}  // namespace imp

#endif  // THIRD_PARTY_IMPRESS_CORE_VIEW_FRAMEWORK_COLLISION_CAPSULE_COLLIDER_H_
