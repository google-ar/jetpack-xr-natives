...
SetNodeEnabledHandler::SetNodeEnabledHandler() {}

Future<absl::Status> SetNodeEnabledHandler::HandleMessage(
    const SetNodeEnabledRequest& message) {
  if (!message.target) {
    return Future<absl::Status>(absl::InvalidArgumentError("Invalid Node ID"));
  }
  message.target->SetEnabled(message.enabled);
