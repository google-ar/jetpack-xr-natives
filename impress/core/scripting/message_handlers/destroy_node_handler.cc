...
DestroyNodeHandler::DestroyNodeHandler(BaseView& base_view)
    : view_(base_view) {}

Future<absl::Status> DestroyNodeHandler::HandleMessage(
    const DestroyNodeRequest& message) {
  if (!message.target) {
    return Future<absl::Status>(absl::InvalidArgumentError("Invalid Node"));
  }

  view_.DestroyNode(message.target);
