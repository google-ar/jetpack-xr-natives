...
#ifndef THIRD_PARTY_IMPRESS_CORE_PHYSICS_CONSTRAINTS_GENERIC_6DOF_CONSTRAINT_H_
#define THIRD_PARTY_IMPRESS_CORE_PHYSICS_CONSTRAINTS_GENERIC_6DOF_CONSTRAINT_H_
...
#include "core/physics/constraints/generic_6dof_constraint_state.proto.imp.h"
...
// Implements a general purpose (with 6 degrees of freedom) constraint which
// gives you complete control over the movement and rotation of the connected
// rigid bodies. You can configure the linear and angular limits individually
// for all the axes. Using zero value limits for an axis will lock the axis.
// Using lower limit greater than upper limit will make the movement free on
// that axis or around it.
class Generic6DofConstraint : public BaseConstraint, public Component {
 public:
  ...
  // A generic 6dof constraint allows rotation around all three axes and
  // translation along all three axes.
  // It allows control over rotation and translation by specifying the limits
  // for each axis.

  // Specify the connected node and whether to auto-configure the pivots and
  // axis. It will perform one of the following:
  //
  // 1. Constructs a generic 6dof constraint between two rigid bodies if the
  // owner node of this component has a RigidBody component.
  //
  // (or) 2. Constructs a world-locked generic 6dof constraint if the owner node
  ...
  // Sets the limits on the linear and angular axes.
  // Setting both lower and upper limits to zero (or to the same value) will
  // lock the axis. Setting lower limit greater than upper limit will make the
  // movement free.
  // The angular limits are in degrees.
  void SetLinearLimits(float3 lower_limit, float3 upper_limit);
  void SetAngularLimits(float3 lower_limit, float3 upper_limit);

  // Returns the current limits
  float3 GetLowerLinearLimit() const;
  float3 GetUpperLinearLimit() const;
  float3 GetLowerAngularLimit() const;
  float3 GetUpperAngularLimit() const;

  // Returns the bullet constraint. Used for testing.
 ...
 private:
  ...
  Generic6DofConstraintState state_;
  std::unique_ptr<btGeneric6DofConstraint> bt_constraint_;

 public:
  using IsfInfo =
      IsfInfo<&Generic6DofConstraint::state_, IsfDependencies<RigidBody>>;
};
