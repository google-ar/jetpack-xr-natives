...
absl::StatusOr<imp::FlatBufferAccess<const imp::schemas::CompileResponse>>
MaterialCompilerClient::CompileMaterial(absl::string_view material_string) {
  flatbuffers::FlatBufferBuilder builder;
  auto command_offset = imp::schemas::CreateCompileRequest(
      builder,
      builder.CreateString(material_string.data(), material_string.size()));
  auto request_offset = imp::schemas::CreateRequest(
      builder, imp::schemas::RequestType::CompileRequest,
...
void MaterialCompilerClient::Close() {
  ...
  flatbuffers::FlatBufferBuilder builder;
  auto request_offset = imp::schemas::CreateRequest(
      builder, imp::schemas::RequestType::CloseRequest,
      imp::schemas::CreateCloseRequest(builder).Union());
  builder.Finish(request_offset);
...
MaterialCompilerClient::GetCompiledMaterialResponse() {
  {
    ...
    if (!verifier.VerifyBuffer<imp::schemas::Response>()) {
      return absl::InternalError("Failed to validate response.");
    }
    const imp::schemas::Response* response =
        flatbuffers::GetRoot<imp::schemas::Response>(message_buffer);

    // If there is an error, return it as an Error to continue error
    // propagation.
    if (response->response_type() ==
        imp::schemas::ResponseType::ErrorResponse) {
      auto error = response->response_as<imp::schemas::ErrorResponse>();
    ...
    return imp::FlatBufferAccess<const imp::schemas::CompileResponse>(
        response->response_as<imp::schemas::CompileResponse>(),
        imp::BufferAccess{std::move(message), message_size});
  }
}
}  // namespace imp_material_compiler
