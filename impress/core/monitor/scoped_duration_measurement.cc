...
ScopedDurationMeasurement::ScopedDurationMeasurement(Monitor* monitor,
                                                     absl::string_view name)
    : monitor_(monitor) {
  id_ = monitor_->GetMeasurementId(name);
  if (!id_) {
    id_ = monitor_->AddMeasurement(
        std::make_unique<DurationMeasurementData>(monitor_, name));
  }
...
ScopedDurationMeasurement::~ScopedDurationMeasurement() {
  static_cast<DurationMeasurementData*>(monitor_->GetMeasurementData(id_))
      ->EndSample();
}

void ScopedDurationMeasurement::AddHistogram(Monitor& monitor,
                                             absl::string_view name,
...
void ScopedDurationMeasurement::CancelSample() {
  static_cast<DurationMeasurementData*>(monitor_->GetMeasurementData(id_))
      ->CancelSample();
}
