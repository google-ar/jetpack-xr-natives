...
#ifndef THIRD_PARTY_IMPRESS_CORE_NCSB_NODE_DEFINITION_H_
#define THIRD_PARTY_IMPRESS_CORE_NCSB_NODE_DEFINITION_H_

// This is the definition of the core Impress Node API.
// Users should include node.h and/or node_handle.h instead.
// IWYU pragma: private, include "third_party/impress/core/ncsb/node_handle.h"
...
class Node final {
 public:
  ...
  // Returns a ComponentHandle<T>, an absl::StatusOr<ComponentHandle<T>>, or a
  // Future<ComponentHandle<T>>.
  template <typename T, typename... Args>
  component_traits::AddResult<T, Args...> AddComponent(Args&&... args);
  ...
  // Setup(Args..).
  template <typename T, typename... Args>
  component_traits::AddWithStateResult<T, Args...> AddComponentWithState(
  ...
  template <typename T, typename... Args>
  component_traits::AddResult<T, Args...> GetOrAddComponent(Args&&... args);
  ...
  // are ignored.
  template <typename T, typename... Args>
  component_traits::AddWithStateResult<T, Args...> GetOrAddComponentWithState(
      typename T::IsfInfo::StateT state, Args&&... args);
  ...
  //
  // Calling this function causes the node to override group inheritance from
  // its ancestors. Future changes to ancestor groups will not affect this
  // node's groups until ClearGroups() is called.
  ...
  //
  // Note: This method allocates a new vector, which can be expensive. Consider
  // using GetChildrenRange() if you only need to iterate over the children.
  std::vector<NodeHandle> GetChildren() const;

  // Returns a lightweight view over the children of this node.
  //
  // This allows for iterating over children (e.g., using a range-for loop) or
  // looking up the child count directly, avoiding the performance and memory
  // overhead of allocating a full std::vector.
  //
  // Warning: Structurally mutating this node's child graph (adding, destroying,
  // or re-parenting a child) instantly invalidates any active iterators mapped
  // under this range.
  NodeChildrenRange GetChildrenRange() const;
  ...
  DispatcherConnection Connect(Fn&& handler);
  ...
  DispatcherConnection Connect(Fn&& handler, Owner owner);
...
#endif  // THIRD_PARTY_IMPRESS_CORE_NCSB_NODE_DEFINITION_H_
