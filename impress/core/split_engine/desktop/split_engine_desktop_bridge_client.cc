...
absl::Status SplitEngineDesktopBridgeClient::SendRequest(
  ...
  stub_->async()->SendRequest(
      &args->context, &args->request, &args->response,
      [args, callback = std::move(callback)](grpc::Status status) {
        
        // TODO: (broken link) - change the callback to take a span to avoid the
        // copy here.
        callback(std::vector<uint8_t>(args->response.data().begin(),
                                      args->response.data().end()));
      });
