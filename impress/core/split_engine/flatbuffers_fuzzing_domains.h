...
#ifndef THIRD_PARTY_GOOGLEFUZZTEST_INTERNAL_DOMAINS_FLATBUFFERS_DOMAIN_IMPL_H_
#define THIRD_PARTY_GOOGLEFUZZTEST_INTERNAL_DOMAINS_FLATBUFFERS_DOMAIN_IMPL_H_
...
using IRObject =
    decltype(static_cast<fuzztest::Domain<int>*>(nullptr)->SerializeCorpus(
        typename fuzztest::Domain<int>::corpus_type{}));
  ...
  uint64_t MutateSelectedField(
    ...
    for (const auto* field : *object_->fields()) {
      ...
      if (field_counter == selected_field_index + 1) {
        VisitFlatbufferField(
            schema_, field,
            MutateVisitor{Self(), prng, metadata, only_shrink, val});
  ...
  struct PrinterVisitor {
    ...
    void Visit(const reflection::Field* /*absl_nonnull*/  field) const {
      ...
      if constexpr (std::is_same_v<T, FlatbuffersVectorTag<uint8_t>> ||
        ...
        if constexpr (std::is_same_v<T, FlatbuffersVectorTag<uint8_t>> ||
                      std::is_same_v<T, FlatbuffersArrayTag<uint8_t>>) {
          auto inner_corpus = object_corpus.GetAs<typename ContainerOfImpl<
              std::vector<uint8_t>, ArbitraryImpl<uint8_t>>::corpus_type>();
          ...
          auto inner_corpus = object_corpus.GetAs<typename ContainerOfImpl<
              std::vector<uint8_t>,
              FlatbuffersEnumDomainImpl<uint8_t>>::corpus_type>();
          auto enum_object =
...
#endif  // THIRD_PARTY_GOOGLEFUZZTEST_INTERNAL_DOMAINS_FLATBUFFERS_DOMAIN_IMPL_H_
