...
SplitEngineMaterial::RequestBuiltInMaterial(
...
void SplitEngineMaterial::MarkParametersDirty(bool dirty) const {
...
bool SplitEngineMaterial::AreParametersDirty() const {
  SplitEngineMaterialUpdater& updater =
      view_.GetRegistry().GetOrCreate<SplitEngineMaterialUpdater>(view_);
...
void SplitEngineMaterialUpdater::Update(const FrameTime& frame_time) {
  for (const SplitEngineMaterial* material : dirty_) {
    material->UpdateParameters();
  }
  dirty_.clear();
}

void SplitEngineMaterialUpdater::RemoveMaterial(SplitEngineMaterial* material) {
  dirty_.erase(material);
}

void SplitEngineMaterialUpdater::MarkParametersDirty(
    const SplitEngineMaterial* material, bool dirty) {
  if (dirty) {
...
bool SplitEngineMaterialUpdater::AreParametersDirty(
    const SplitEngineMaterial* material) const {
