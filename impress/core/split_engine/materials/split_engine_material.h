...
#ifndef THIRD_PARTY_IMPRESS_CORE_SPLIT_ENGINE_MATERIALS_SPLIT_ENGINE_MATERIAL_H_
#define THIRD_PARTY_IMPRESS_CORE_SPLIT_ENGINE_MATERIALS_SPLIT_ENGINE_MATERIAL_H_
...
// A SplitEngineMaterial is used when an Impress application uses a material
// through Split Engine. This is used as a base for both built-in and custom
// materials.
//
// Note: This class and all derived classes can be be used in local mode, where
// there is no Split Engine bridge.
class SplitEngineMaterial : public Material {
 public:
  // Creates a placeholder material on the app side.
  static Future<OwnedMaterialPtr> CreatePlaceholderMaterial(BaseView& view);

  explicit SplitEngineMaterial(BaseView& view, OwnedMaterialPtr material);
 ...
 private:
  ABSL_ATTRIBUTE_NOINLINE void LogParameterNotSupportedError(
      absl::string_view parameter_name);

  template <int&... ExplicitArgumentBarrier, typename T>
  void SetParameterInternal(absl::string_view parameter_name, const T& value);

  template <int&... ExplicitArgumentBarrier, typename T>
  void SetParameterInternal(absl::string_view parameter_name,
                            absl::Span<const T> values);
};

template <int&... ExplicitArgumentBarrier, typename T>
void SplitEngineMaterial::SetParameterInternal(absl::string_view parameter_name,
                                               const T& value) {
...
void SplitEngineMaterial::SetParameterInternal(absl::string_view parameter_name,
  ...
  if (SplitEngineSerializer* serializer = view_.GetSplitEngineSerializer()) {
    ...
    material_->SetParameter(parameter_name, values);
  }
}

}  // namespace imp::split_engine

#endif  // THIRD_PARTY_IMPRESS_CORE_SPLIT_ENGINE_MATERIALS_SPLIT_ENGINE_MATERIAL_H_
