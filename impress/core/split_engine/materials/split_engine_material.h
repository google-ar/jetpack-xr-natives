...
#ifndef VR_ANDROID_XR_SPLIT_ENGINE_MATERIALS_SPLIT_ENGINE_MATERIAL_H_
...
// A base class for Split Engine materials on the app-side. This class
// encapsulates the boilerplate for serializing material parameters.
class SplitEngineMaterial {
 public:
...
class SplitEngineMaterialUpdater
 ...
 public:
  ...
  void MarkParametersDirty(const SplitEngineMaterial* material,
                           bool dirty = true);
  bool AreParametersDirty(const SplitEngineMaterial* material) const;
  void RemoveMaterial(SplitEngineMaterial* material);

 private:
  // Set of SplitEngineMaterials whose parameters have changed this frame.
  RobinSet<const SplitEngineMaterial*> dirty_;
};

}  // namespace imp::split_engine

#endif  // VR_ANDROID_XR_SPLIT_ENGINE_MATERIALS_SPLIT_ENGINE_MATERIAL_H_
