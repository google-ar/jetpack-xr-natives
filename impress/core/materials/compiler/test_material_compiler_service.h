 ...
 */
#ifndef THIRD_PARTY_IMPRESS_CORE_MATERIALS_COMPILER_TEST_MATERIAL_COMPILER_SERVICE_H_
#define THIRD_PARTY_IMPRESS_CORE_MATERIALS_COMPILER_TEST_MATERIAL_COMPILER_SERVICE_H_
...
class TestMaterialCompilerService {
 public:
  ...
  MOCK_METHOD(void, OnRequest, (const schemas::Request*));
 ...
 private:
  void OnMessage(std::unique_ptr<uint8_t[]> message, size_t size) {
    ...
    ASSERT_TRUE(verifier.VerifyBuffer<schemas::Request>());
    const schemas::Request* request =
        flatbuffers::GetRoot<schemas::Request>(message.get());
...
#endif  // THIRD_PARTY_IMPRESS_CORE_MATERIALS_COMPILER_TEST_MATERIAL_COMPILER_SERVICE_H_
