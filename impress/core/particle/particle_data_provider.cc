...
float2 ParticleDataProvider::GetVector2f(int32_t index) const {
  return float2(GetFloat(index), GetFloat(index + 1));
}

void ParticleDataProvider::SetVector2f(int32_t index, const float2& value) {
  SetFloat(index, value.x);
  SetFloat(index + 1, value.y);
}

float3 ParticleDataProvider::GetVector3f(int32_t index) const {
  return float3(GetFloat(index), GetFloat(index + 1), GetFloat(index + 2));
}

void ParticleDataProvider::SetVector3f(int32_t index, const float3& value) {
  SetFloat(index, value.x);
  SetFloat(index + 1, value.y);
  SetFloat(index + 2, value.z);
}

quatf ParticleDataProvider::GetQuatf(int32_t index) const {
  // Restoring from (w, x, y, z) to match the quaternion constructor.
  return quatf(GetFloat(index), GetFloat(index + 1), GetFloat(index + 2),
               GetFloat(index + 3));
}

void ParticleDataProvider::SetQuatf(int32_t index, const quatf& value) {
