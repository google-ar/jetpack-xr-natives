...
constexpr absl::string_view kAnimationsLengthToken = "animations.length";
}  // namespace

std::vector<TokenParser> AnimationsLengthPointerDeclaration::GetTokenParsers()
    const {
  return {std::string(kAnimationsLengthToken)};
}

absl::StatusOr<PointerValue> AnimationsLengthPointerDeclaration::GetValue(
  ...
  return static_cast<int>(gltf_renderer->GetGltfAsset()->AnimationCount());
}

absl::Status AnimationsLengthPointerDeclaration::SetValue(
    NodeHandle gltf_model, absl::Span<const ParsedToken> parsed_tokens,
    PointerValue value) const {
  return absl::FailedPreconditionError(
      "Setting animations length is not supported.");
}
