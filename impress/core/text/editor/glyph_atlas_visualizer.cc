...
// Percentage of the atlas that is padded for better UX
constexpr float kAtlasPadding = 0.15f;
#else
constexpr int kPanelHeight = 300;
constexpr float kAtlasPadding = 0.1f;
#endif
...
GlyphAtlasVisualizer::GlyphAtlasVisualizer(BaseView& view,
...
GlyphAtlasVisualizer::~GlyphAtlasVisualizer() = default;

bool GlyphAtlasVisualizer::HasContent() const {
  return provider_.get_texture_func() != nullptr;
}

void GlyphAtlasVisualizer::DrawImGui() {
  Texture* texture = provider_.get_texture_func();
  Draw(texture);
}

void GlyphAtlasVisualizer::Draw(Texture* glyph_atlas_texture) {
  ...
  if (ImPlot::BeginPlot("##Atlas", ImVec2(-1, kPanelHeight),
    ...
    if (ImPlot::IsPlotHovered()) {
      ...
      if (mouse.x > 0 && mouse.x < 1 && mouse.y > 0 && mouse.y < 1) {
        ...
        std::optional<GlyphAtlasInfo> glyph = provider_.get_glyph_info_func(uv);
        if (glyph.has_value()) {
    ...
    if (show_all_glyph_origins_ || show_all_glyph_bounds_) {
      std::vector<GlyphAtlasInfo> glyphs = provider_.get_all_glyph_info_func();
      for (int i = 0; i < glyphs.size(); ++i) {
        const GlyphAtlasInfo& glyph = glyphs[i];
        Rect bounds = glyph.uv;
        bounds.center.y = 1 - bounds.center.y;
...
void GlyphAtlasVisualizer::DrawGlyphBounds(const Rect& bounds) const {
...
void GlyphAtlasVisualizer::DrawGlyphOutline(const Rect& bounds,
                                            int color_idx) const {
  ImDrawList* draw_list = ImPlot::GetPlotDrawList();
  if (!draw_list) {
    return;
...
void GlyphAtlasVisualizer::DrawGlyphOrigin(const Rect& bounds,
...
void GlyphAtlasVisualizer::DrawToolTip(const GlyphAtlasInfo& info,
