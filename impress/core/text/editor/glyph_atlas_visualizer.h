...
#ifndef THIRD_PARTY_IMPRESS_CORE_TEXT_EDITOR_GLYPH_ATLAS_VISUALIZER_H_
#define THIRD_PARTY_IMPRESS_CORE_TEXT_EDITOR_GLYPH_ATLAS_VISUALIZER_H_
...
class GlyphAtlasVisualizer : public Widget, public imp::Rememberer {
 public:
  ...
  struct AtlasDataProvider {
    ...
    Invocable<std::optional<GlyphAtlasInfo>(const float2&)> get_glyph_info_func;
    Invocable<float()> get_utilization_func;
    Invocable<std::vector<GlyphAtlasInfo>()> get_all_glyph_info_func;
  };

  explicit GlyphAtlasVisualizer(BaseView& view, AtlasDataProvider provider);
  ~GlyphAtlasVisualizer() override;
 ...
 private:
  ...
  void DrawToolTip(const GlyphAtlasInfo& info,
...
#endif  // THIRD_PARTY_IMPRESS_CORE_TEXT_EDITOR_GLYPH_ATLAS_VISUALIZER_H_
