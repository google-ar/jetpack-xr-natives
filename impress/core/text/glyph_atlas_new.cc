...
AsyncScopedCanvas* GlyphAtlasNew::GetOrStartDrawing(
...
GlyphAtlasNew::GlyphAtlasNew(BaseView& view, Config config)
    : GlyphAtlasNew::GlyphAtlasNew(
          view, AsyncCanvasSourceFactory::Create(view.GetContext()), config) {}

GlyphAtlasNew::GlyphAtlasNew(BaseView& view,
...
void GlyphAtlasNew::AddFont(absl::string_view font_name,
...
const GlyphAtlasNew::GlyphInfo& GlyphAtlasNew::GetOrAddGlyphInfo(
    GlyphEmulator::Glyph& glyph, const CanvasOptionsGlyphKey& glyph_key) {
  const GlyphAtlasNew::GlyphInfo* /*absl_nullable*/ glyph_info =
...
void GlyphAtlasNew::DrawAllGlyphsToCanvas() {
  ScopedCanvas* canvas;
  {
    absl::MutexLock lock(&canvas_mutex_);
...
Future<absl::Status> GlyphAtlasNew::DrawGlyphsToCanvasAsync(
...
absl::Status GlyphAtlasNew::DrawGlyphsToCanvas(
...
std::optional<AtlasPacker::ScopedAtlasEntry> GlyphAtlasNew::TryAddAtlasEntry(
    uint2 atlas_entry_size) {
...
void GlyphAtlasNew::ClearUnusedGlyphs() {
  absl::MutexLock glyph_map_lock(&glyph_map_mutex_);
  absl::MutexLock canvas_lock(&canvas_mutex_);
  ...
  for (auto glyph_itr = glyph_map_.begin(); glyph_itr != glyph_map_.end();) {
    ...
    if (copy_glyph_itr->second.ref_counter.GetCount() == 0) {
      ...
      if (canvas_source_->IsFeatureSupported(
        ...
        const GlyphAtlasNew::GlyphInfo& glyph_info = copy_glyph_itr->second;
...
size_t GlyphAtlasNew::GetNumCachedGlyphs() const {
  absl::MutexLock glyph_map_lock(&glyph_map_mutex_);
  return glyph_map_.size();
}

GlyphAtlas::Glyph GlyphAtlasNew::GlyphInfoToGlyph(const GlyphInfo& glyph_info,
                                                  float advance_width,
...
void GlyphAtlasNew::DrawGlyphToCanvas(ScopedCanvas& canvas,
...
std::string GlyphAtlasNew::ToString(
...
Texture* GlyphAtlasNew::GetTexture() {
  if (Executor::CurrentExecutor() != Executor::ForegroundExecutor()) {
...
void GlyphAtlasNew::PrepareToUpdateTexture() {
...
float GlyphAtlasNew::GetAtlasUtilization() const {
  float utilization;
  {
    absl::MutexLock lock(&atlas_packer_mutex_);
...
GlyphEmulator& GlyphAtlasNew::GetGlyphEmulator() { return glyph_emulator_; }

#if IMP_RUNTIME(DEV)
std::optional<editor::GlyphAtlasVisualizer::GlyphAtlasInfo>
GlyphAtlasNew::GetGlyphAtlasInfoAt(const float2& uv) const {
...
GlyphAtlasNew::GetAllGlyphInfo() const {
