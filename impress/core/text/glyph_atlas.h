...
class GlyphAtlas : public Rememberer {
 public:
  ...
  struct TextOptions {
    ...
    // GlyphAtlas::AddFont, or a SystemFontParams object which describes the
  ...
  GlyphAtlas(BaseView& view, Config config = kDefaultConfig);
  GlyphAtlas(BaseView& view, std::unique_ptr<AsyncCanvasSource> canvas_source,
  ...
  //
  // This function is thread-safe.
  size_t GetNumCachedGlyphs() const ABSL_LOCKS_EXCLUDED(glyph_map_mutex_);
  ...
  GetGlyphAtlasInfoAt(const float2& uv) const;
 ...
 private:
  ...
  // atlas.
  using GlyphMap = RobinMap<CanvasOptionsGlyphKey, GlyphInfo>;
  ...
  BreakIntoGlyphs(absl::string_view text,
                  const ScopedCanvas::TextOptions& canvas_options);
  ...
  // added to this atlas.
  absl::Nullable<const GlyphInfo*> GetGlyphInfo(
