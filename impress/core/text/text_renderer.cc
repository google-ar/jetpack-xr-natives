...
Future<absl::Status> TextRenderer::UpdateMeshesAndMaterials() {
  if (glyph_atlas_) {
    return UpdateMeshesAndMaterialsFromAtlas();
  } else {
    return UpdateMeshesAndMaterialsFromSlicedAtlas();
  }
}

Future<absl::Status> TextRenderer::UpdateMeshesAndMaterialsFromAtlas() {
  ...
  if (options.precomputed_metrics.has_value()) {
    ...
    font_info_future = glyph_atlas_->GetFontInfo(options);
  }
  ...
  return text_material_future
      ...
      .Then([this](std::tuple<MaterialPtr, std::vector<GlyphAtlas::Glyph>,
                              FontInfo, std::vector<ScopedCanvas::GlyphGroup>,
        ...
        atlas_glyphs_ = std::move(glyphs);
        if (atlas_glyphs_.empty()) {
...
Future<absl::Status> TextRenderer::UpdateMeshesAndMaterialsFromSlicedAtlas() {
  // TODO (broken link) Ensure that the color space here is correct
  GlyphEmulator::TextOptions options = {
      .font_params = state_.font_params,
      .font_size_pixels = state_.font_size_pixels,
  ...
  Future<FontInfo> font_info_future = sliced_glyph_atlas_->GetFontInfo(options);

  Future<std::vector<SlicedGlyphAtlas::Glyph>> glyphs_future =
      sliced_glyph_atlas_->GetGlyphs(state_.text, options);
  ...
  if (text_layout_provider_.has_value()) {
    combined_character_indices_future =
        sliced_glyph_atlas_->GetCombinedCharacterGroups(state_.text, options);
  }

  Future<GlyphEmulator::SuperSampleInfo> super_sample_info_future =
      sliced_glyph_atlas_->GetSuperSampleInfo(options.force_non_separable);

  return text_material_future
      .Merge(glyphs_future, font_info_future, combined_character_indices_future,
             super_sample_info_future)
      .Then([this](std::tuple<MaterialPtr, std::vector<SlicedGlyphAtlas::Glyph>,
                              FontInfo, std::vector<ScopedCanvas::GlyphGroup>,
                              GlyphEmulator::SuperSampleInfo>
                       result) -> absl::Status {
        font_info_ = std::get<2>(result);
        ...
        MaterialPtr& text_material = std::get<0>(result);
        std::vector<SlicedGlyphAtlas::Glyph> glyphs = std::get<1>(result);
        sliced_atlas_glyphs_ = std::move(glyphs);
        if (sliced_atlas_glyphs_.empty()) {
          return absl::InvalidArgumentError(
              "Unable to update text geometry with empty glyphs for text: " +
              state_.text);
        }
...
void TextRenderer::RecalculateMesh(bool force_regenerate_mesh) {
  size_t glyph_count =
      std::max(sliced_atlas_glyphs_.size(), atlas_glyphs_.size());
...
TextRenderer::CalculateGlyphPlacements(
  ...
  if (!text_layout_provider_.has_value()) {
    float glyph_x_offset = 0.0f;
    for (int glyph_index = 0; glyph_index < glyphs.size(); ++glyph_index) {
      const SlicedGlyphAtlas::Glyph& glyph = glyphs[glyph_index];
      float glyph_center = glyph.advance_width * 0.5f;
...
TextRenderer::CalculateGlyphVerticesAndVisualBounds() const {
  if (glyph_atlas_) {
    return CalculateGlyphVerticesAndVisualBounds(atlas_glyphs_);
  } else {
    return CalculateGlyphVerticesAndVisualBounds(sliced_atlas_glyphs_);
  }
}
...
void TextRenderer::RenderGlyphPass(int vertex_offset, int index_offset,
  ...
  if (glyph_atlas_) {
    RenderAtlasGlyphPass(vertex_offset, index_offset, glyph_vertices,
                         has_stroke, mesh_data);
  } else {
    RenderSlicedAtlasGlyphPass(vertex_offset, index_offset, glyph_vertices,
                               has_stroke, mesh_data);
  }
}

void TextRenderer::RenderAtlasGlyphPass(
  ...
  for (int glyph_index = 0; glyph_index < atlas_glyphs_.size(); ++glyph_index) {
    const GlyphAtlas::Glyph& glyph = atlas_glyphs_[glyph_index];
    for (int vertex_index = 0; vertex_index < kQuadVertices.size();
         ++vertex_index) {
      const QuadVertex& quad_vertex = kQuadVertices[vertex_index];
      int glyph_vert_index =
...
void TextRenderer::RenderSlicedAtlasGlyphPass(
    int vertex_offset, int index_offset,
    const std::vector<float3>& glyph_vertices, bool has_stroke,
    MeshData& mesh_data) {
  float glyph_render_multiplier = GetMetersPerPixel();
...
void TextRenderer::CalculateTextDimensionsAndPivot() {
  if (glyph_atlas_) {
    CalculateAtlasTextDimensionsAndPivot();
  } else {
    CalculateSlicedAtlasTextDimensionsAndPivot();
  }
}

void TextRenderer::CalculateAtlasTextDimensionsAndPivot() {
  float glyph_x_offset = 0.0f;
...
void TextRenderer::CalculateSlicedAtlasTextDimensionsAndPivot() {
  float glyph_x_offset = 0.0f;
...
void TextRenderer::CalculatePath() {
  if (glyph_atlas_) {
    ...
    CalculatePathSlicedAtlas();
  }
}

void TextRenderer::CalculatePathAtlas() {
  if (!text_layout_provider_.has_value()) {
    return;
  }
  ...
  std::vector<GlyphPlacement> glyph_placements =
      CalculateGlyphPlacements(atlas_glyphs_);
  for (int glyph_index = 0; glyph_index < atlas_glyphs_.size(); ++glyph_index) {
    const GlyphAtlas::Glyph& glyph = atlas_glyphs_[glyph_index];
    float3 translation = glyph_placements[glyph_index].position;
    mat2f rotation = glyph_placements[glyph_index].rotation;
...
void TextRenderer::CalculatePathSlicedAtlas() {
  if (!text_layout_provider_.has_value()) {
    return;
  }
  ...
  std::vector<GlyphPlacement> glyph_placements =
      CalculateGlyphPlacements(sliced_atlas_glyphs_);
  for (int glyph_index = 0; glyph_index < sliced_atlas_glyphs_.size();
       ++glyph_index) {
    const SlicedGlyphAtlas::Glyph& glyph = sliced_atlas_glyphs_[glyph_index];
    float3 translation = glyph_placements[glyph_index].position;
