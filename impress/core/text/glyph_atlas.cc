...
// It's exceedingly rare for glyphs to come close to or exceed this size, and
// glyphs this large cause inefficiencies in the atlas packer.
constexpr float kLargeGlyphSize = 100.0f;
...
constexpr ScopedCanvas::TextOptions kTextOptions{
    .size_pixels = GlyphAtlas::kDefaultFontSizePixels,
...
AsyncScopedCanvas* GlyphAtlas::GetOrStartDrawing(
  ...
  canvas_ = canvas_source_->StartDrawing(atlas_texture_size_, draw_mode);
...
GlyphAtlas::GlyphAtlas(BaseView& view, Config config)
    : GlyphAtlas::GlyphAtlas(view, AsyncCanvasSourceFactory::Create(view),
                             config) {}
            ...
            .Then([this, text = std::string(text),
              ...
              return BreakIntoGlyphs(text, canvas_options)
                  .Then([this, canvas_options](
                    ...
                    Future<
                        absl::flat_hash_map<GlyphAtlas::GlyphKeyOrGlyphString,
...
Future<std::unique_ptr<std::vector<GlyphAtlas::GlyphAdvance>>>
GlyphAtlas::BreakIntoGlyphs(absl::string_view text,
    ...
    return canvas_source_->GetTextGlyphs(text, canvas_options)
        .Then([font_holder_name](
          ...
          for (ScopedCanvas::GlyphAdvance& canvas_glyph_advance :
            ...
            glyph_advances->push_back(GlyphAtlas::GlyphAdvance{
        ...
        [chunks = std::move(chunks), contains_rtl,
         canvas_options](std::vector<std::vector<float>> widths) {
          ...
          for (int i = 0; i < chunks.size(); i++) {
            GlyphAtlas::GetGlyphsForChunk(chunks[i], widths[i], canvas_options,
                                          *glyph_advances);
          }
...
void GlyphAtlas::GetGlyphsForChunk(
    const Chunk& chunk, const std::vector<float>& advance_widths,
    const ScopedCanvas::TextOptions& canvas_options,
    std::vector<GlyphAtlas::GlyphAdvance>& out_glyph_advances) {
  ...
  if (chunk.codepoint_count != advance_widths.size()) {
    IMP_LOG(imp::FATAL) << "GlyphAtlas is unable to generate glyphs for text "
...
absl::Nullable<const GlyphAtlas::GlyphInfo*> GlyphAtlas::GetGlyphInfo(
...
Future<absl::flat_hash_map<GlyphAtlas::GlyphKeyOrGlyphString,
                           ScopedCanvas::TextMetrics>>
GlyphAtlas::MeasureGlyphs(const std::vector<GlyphAdvance>& glyphs,
  ...
  for (const GlyphAdvance& glyph_advance : glyphs) {
    absl::Nullable<const GlyphAtlas::GlyphInfo*> glyph_info = GetGlyphInfo(
...
const GlyphAtlas::GlyphInfo& GlyphAtlas::GetOrAddGlyphInfo(
    ...
    const absl::flat_hash_map<GlyphKeyOrGlyphString, ScopedCanvas::TextMetrics>&
        measurement_map) {
  absl::Nullable<const GlyphAtlas::GlyphInfo*> glyph_info =
  ...
  if (!atlas_entry) {
    // TODO: Clean up this error logging once (broken link) is fixed.
    if (atlas_entry_size.x > kLargeGlyphSize ||
        atlas_entry_size.y > kLargeGlyphSize) {
      IMP_LOG(imp::ERROR) << "Unable to find space in Glyph Atlas for glyph \""
                 << ToString(glyph_advance.glyph)
                 << ", likely due to the glyph being too long.";
    } else if (atlas_entry_size.x > kLargeGlyphSize &&
               atlas_entry_size.y > kLargeGlyphSize) {
      IMP_LOG(imp::ERROR) << "Unable to find space in Glyph Atlas for glyph \""
                 << ToString(glyph_advance.glyph)
                 << ", likely due to a large glyph size.";
    } else {
      IMP_LOG(imp::ERROR) << "Unable to find space in Glyph Atlas for glyph \""
                 << ToString(glyph_advance.glyph)
...
void GlyphAtlas::ClearUnusedGlyphs() {
  ...
  for (auto glyph_itr = glyph_map_.begin(); glyph_itr != glyph_map_.end();) {
    if (glyph_itr.value().ref_counter.GetCount() == 0) {
      ...
      if (canvas_source_->IsFeatureSupported(
        ...
        const GlyphAtlas::GlyphInfo& glyph_info = glyph_itr.value();
      ...
      glyph_itr = glyph_map_.erase(glyph_itr);
    } else {
      glyph_itr++;
    }
  }
...
void GlyphAtlas::DrawGlyphKeyOrGlyphString(
...
GlyphAtlas::CanvasOptionsFromGlyphAtlasOptions(const TextOptions& options) {
...
std::string GlyphAtlas::ToString(const GlyphKeyOrGlyphString& glyph) {
  if (absl::holds_alternative<GlyphKey>(glyph)) {
...
bool GlyphAtlas::ShouldSuperSample() const {
#if IMP_PLATFORM(WASM)
...
float2 GlyphAtlas::GetSubpixelRenderRatio() const {
#if IMP_PLATFORM(WASM)
