...
#include "core/loader/provider/provider_gltf_test_helpers.h"
...
ProviderGltfTestHelpers::ProviderGltfTestHelpers() {
  // Resource setup.
...
std::string ProviderGltfTestHelpers::GetSampleAssetsDirectory() {
...
std::string ProviderGltfTestHelpers::SampleAsset(std::string friendly_path) {
  return GetSampleAssetsDirectory() + friendly_path;
}

OptionalError ProviderGltfTestHelpers::CreateProvider(
    std::string friendly_path) {
  return CreateProvider(friendly_path, {});
}

OptionalError ProviderGltfTestHelpers::CreateProvider(std::string friendly_path,
                                                      LoaderOptions options) {
  auto path = SampleAsset(friendly_path);
  BufferAccess access;
  MP_RETURN_IF_ERROR(LoadBinary(path, &access));
  MP_ASSIGN_OR_RETURN(provider_,
                   Provider::Create(path, std::move(access), options));
  return NoError();
}

const schemas::LoadedModel* ProviderGltfTestHelpers::GetLoadedModel() {
...
void ProviderGltfTestHelpers::AddMissingResources(
...
OptionalError ProviderGltfTestHelpers::LoadWithResources(
