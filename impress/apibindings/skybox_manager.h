...
class SkyboxManager {
 public:
  explicit SkyboxManager(ImpressApiView& view);
  ...
  // asset.
  virtual absl::Status ReleaseImageBasedLightingAsset(std::intptr_t ibl_token);

  // Sets the preferred IBL asset to be used by the system.
  virtual absl::Status SetEnvironmentLight(std::intptr_t ibl_token);

  // Clears the preferred IBL asset to be used by the system.
  virtual absl::Status ClearEnvironmentLight();

  // Disposes all IBL assets.
