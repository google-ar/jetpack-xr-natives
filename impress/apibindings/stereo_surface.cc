...
absl::Status StereoSurface::Setup(MediaStereoMode stereo_mode) {
  ...
  MP_ASSIGN_OR_RETURN(surface_,
                   AndroidExternalTextureSurface::Create(
                       GetView(), ContentSecurityLevel::kNone, view_types));
  ...
  material_future_ = android_xr::JxrMediaMaterial::Create(GetView()).Then(
      [this,
       stereo_mode](std::unique_ptr<android_xr::JxrMediaMaterial> material) {
...
void StereoSurface::SetCanvasShape(const CanvasShape& canvas_shape) {
  ...
  if (u != nullptr) {
    ...
    // TODO: (broken link) - Update this to return absl::Status instead of
    // crashing.
    
    return;  // early return to avoid avoid spuriously enabling the mesh.
  }
...
android::Surface* StereoSurface::GetSurface() { return surface_->GetSurface(); }
